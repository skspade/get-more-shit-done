...
- [x] **SAFE-01**: Implement per-step-type `maxTurns` limits via `TURNS_CONFIG` (discuss:100, plan:150, execute:300, verify:100, debug:50, audit:100, uat:150, completion:50), configurable via config.json
- [x] **SAFE-02**: Implement optional `maxBudgetUsd` per-step cost cap via config key `autopilot.max_budget_per_step_usd`
...
- [x] **CALL-02**: Wire `runAgentStep()` to all 3 debug retry call sites in `runStepWithRetry()` and `runVerifyWithDebugRetry()`
- [x] **CALL-03**: Update debug retry logic to only trigger on `error_during_execution` subtype (not `error_max_turns` or `error_max_budget_usd`)
...
- [x] **CLN-01**: Delete `runClaudeStreaming()`, `displayStreamEvent()`, quiet-mode CLI branch, and `which('node')` check
- [x] **CLN-02**: Register all new config keys in `config.cjs` (`autopilot.turns.*`, `autopilot.max_budget_per_step_usd`, `autopilot.max_turns_per_step`, `uat.chrome_mcp_enabled`)
...
| SAFE-01 | Phase 101 | Complete |
| SAFE-02 | Phase 101 | Complete |
| SAFE-03 | Phase 98 | Verified |
| SAFE-04 | Phase 98 | Verified |
| CALL-01 | Phase 98 | Verified |
| CALL-02 | Phase 101 | Complete |
| CALL-03 | Phase 101 | Complete |
| MCP-01 | Phase 102 | Pending |
| OBS-01 | Phase 102 | Pending |
| OBS-02 | Phase 102 | Pending |
| CLN-01 | Phase 101 | Complete |
| CLN-02 | Phase 101 | Complete |
